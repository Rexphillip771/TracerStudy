...
if ($login_level == '1') {
    ...
    if ($result && mysqli_num_rows($result) > 0) { // Check if $result contains any rows
        $data = mysqli_fetch_array($result);
        $data_img = $data['student_img'];
    } else {
        // Handle the case where $result doesn't contain any rows
        // Set a default value for $data_img or display an error message
    }
} else if ($login_level == '2') {
    $result = mysqli_query($con,"SELECT * FROM `user_teacher_detail` WHERE teacher_userID = $login_id");
    // Similar handling as above for $result
} else if ($login_level == '3') {
    $result = mysqli_query($con,"SELECT * FROM `user_admin_detail` WHERE admin_userID = $login_id");
    // Similar handling as above for $result
} else {
    // Handle the case for other login levels if needed
}
?>
