...
$data_img = ''; // Initialize $data_img variable to avoid null error
  ...
  <head>
    ...
    <div class="box">
        <ul class="nav nav-tabs">
            <?php 
            $student_sql = mysqli_query($con,"SELECT YEAR(usd.student_year_grad) as student_year_grad,cd.department_ID ,cd.department_name FROM `user_student_detail` usd INNER JOIN cvsu_department cd ON usd.student_department = cd.department_ID WHERE usd.student_ID = $login_id");
            
            if(mysqli_num_rows($student_sql) > 0) { // Check if the query returned any rows
                ...
                while ($data = mysqli_fetch_array($dep)) {
                    if ($data['department_ID'] == $s_department_ID) {
                        ...
                        ?><li><a data-toggle="tab" href="#<?php echo $data['department_ID'] ?>"><?php echo $data['department_name'] ?></a></li><?php
                    }
                }
            } else {
                // Handle the case where the query doesn't return any rows
                echo "No data found.";
            }
            ?>
        </ul>
